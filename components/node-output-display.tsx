  ...
  if (nodeName === "Text Output") {
    return (
      <div className="mt-2 p-2 bg-gray-50 border rounded-md">
        ...
        <div className="text-sm whitespace-pre-wrap">{outputData.displayText || "No output text"}</div>
      </div>
    )
  }
      ...
      <div className="mt-2 p-2 bg-gray-50 border rounded-md max-h-40 overflow-y-scroll pb-2">
        ...
        {outputData.tokenUsage && (
          <div className="mt-2 pt-2 border-t border-gray-200">
            ...
            <div className="text-xs text-gray-500 mt-1">Processing Time: {outputData.processingTime}</div>
          </div>
        )}
      </div>
    )
  ...
  if (nodeName === "If Condition") {
    ...
    return (
      <div className="mt-2 p-2 bg-gray-50 border rounded-md">
        ...
        <div className={`text-sm font-medium ${outputData.true ? "text-green-600" : "text-red-600"}`}>{result}</div>
      </div>
    )
  }
...
function WhatsAppInputNodeDisplay({ nodeName, outputData }: { nodeName: string; outputData: any }) {
  return (
    <div className="mt-2 p-2 bg-gray-50 border rounded-md">
      ...
      <div className="text-sm whitespace-pre-wrap">{outputData.message || "No message content"}</div>
...
function WhatsAppOutputNodeDisplay({ nodeName, outputData }: { nodeName: string; outputData: any }) {
  return (
    <div className="mt-2 p-2 bg-gray-50 border rounded-md">
      <div className="text-xs text-gray-500 mb-1 flex items-center justify-between">
      ...
      {outputData.messageDetails && (
        <>
          ...
          <div className="mt-1 text-xs text-gray-500">Type: {outputData.messageDetails.messageType}</div>
          {outputData.messageId && <div className="mt-1 text-xs text-gray-500">Message ID: {outputData.messageId}</div>}
        </>
      )}
      {outputData.error && <div className="mt-1 text-xs text-red-500">Error: {outputData.error}</div>}
    </div>
  )
}
